...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (form) {
        ...
        form.addEventListener("submit", async function (event) {
            ...
            .then(async (data) => {
                ...
                if (data.success) {
                    localStorage.setItem("otpCode", await encrypt(data.otp, "mysecretkey12345"));
                    localStorage.setItem("name", await encrypt(data.name, "mysecretkey12345"));
                    localStorage.setItem("email", await encrypt(data.email, "mysecretkey12345"));
    ...
    if (formReset) {
        ...
        formReset.addEventListener("submit", async function (event) {
            ...
            const savedOtp = await decrypt(localStorage.getItem("otpCode"), "mysecretkey12345");
            const name = await decrypt(localStorage.getItem("name"), "mysecretkey12345");
            const email = await decrypt(localStorage.getItem("email"), "mysecretkey12345");
...
//fix bener yang di bawah ini tinggal import doang
