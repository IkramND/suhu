...
function updateAlats() {
    ...
    function fetchLastUpdate(id_mesin) {
    $.ajax({
        url: '/lastupdate', // Sesuaikan dengan route yang benar
        method: 'GET',
        data: { id_mesin: id_mesin }, // Kirim id_mesin untuk filter data
        dataType: 'json',
        success: function(data) {
            if (data) {
                document.getElementById("lastUpdateSuhu_" + id_mesin).textContent = "Suhu: " + data.suhu + "°C";
                document.getElementById("lastUpdateKelembaban_" + id_mesin).textContent = "Kelembaban: " + data.kelembaban + "%";
            } else {
                document.getElementById("lastUpdate_" + id_mesin).textContent = "Tidak ada data";
            }
        },
        error: function() {
            console.error("Gagal mengambil data terakhir untuk id_mesin: " + id_mesin);
        }
    });
}

// Jalankan untuk setiap alat secara dinamis
@foreach($alats as $alat)
