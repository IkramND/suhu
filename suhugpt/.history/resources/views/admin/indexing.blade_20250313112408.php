...
<button onclick="fetchData({{ $alat->id_mesin }})" style="background-color: blue; color: white; padding: 4px 8px; border: none; border-radius: 5px; cursor: pointer;">Update</button>
            ...
            <div style="background: red; position: relative; width: 100px; height: 100px; display: flex; flex-direction: column; align-items: flex-end; justify-content: start;">
                ...
                <div style="margin-top: 50px; text-align: right;">
                    <p style="margin: 0;"><span id="lastupdatesuhu_{{$alat->id_mesin}}"></span>1</p>
                    <p style="margin: 0;"><span id="lastupdatekelembaban_{{$alat->id_mesin}}"></span>2</p>
                </div>
            </div>
...
function updateAlats() {
    ...
    function getLastUpdate(id_mesin) {
    $.ajax({
        url: `/lastupdate/${id_mesin}`,
        method: 'GET',
        dataType: 'json',
        success: function(data) {
            if (data.length > 0) {
                let lastData = data[0]; // Ambil data terbaru
                document.getElementById(`lastUpdate_${id_mesin}`).textContent = new Date(lastData.waktu).toLocaleString();
                document.getElementById(`lastupdatesuhu_${id_mesin}`).textContent = `Suhu: ${lastData.suhu}°C`;
                document.getElementById(`lastupdatekelembaban_${id_mesin}`).textContent = `Kelembaban: ${lastData.kelembaban}%`;
            } else {
...
@foreach($alats as $alat)
    getLastUpdate({{ $alat->id_mesin }});
    setInterval(() => getLastUpdate({{ $alat->id_mesin }}), 2000);
@endforeach
