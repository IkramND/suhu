        ...
        <div class="card">
            <div class="pisah" style="grid-template-columns: 4fr 4fr 1fr; display: grid; position: relative;">
                <div>
                    ...
                    <h4>Status :<span id="status_{{ $alat->id_mesin}}" style="color: {{$alat->status == 'Active' ? 'green' : 'red'}}"></span> {{$alat->status}}</h4>
...
function updateAlats{{$alat->id_mesin}}() {
        $.ajax({
            url: '/get-alats',
            method: 'GET',
            dataType: 'json',
