...
function updateAlats() {
    ...
    fetchlastupdate{{$alat->id_mesin}}({{alat->id_mesin}})
    {
        $.ajax({
        url: `/last-update/${id_mesin}`,
        method: 'GET',
        dataType: 'json',
        success: function(data) {
            document.getElementById(`lastupdatesuhu_${id_mesin}`).textContent = data.suhu !== null ? data.suhu + "°C" : "Tidak ada data";
            document.getElementById(`lastupdatekelembaban_${id_mesin}`).textContent = data.kelembaban !== null ? data.kelembaban + "%" : "Tidak ada data";
        }
    });
    }

    fetchLastUpdate("{{ $alat->id_mesin }}");
    setInterval(() => fetchLastUpdate("{{ $alat->id_mesin }}"), 5000);
