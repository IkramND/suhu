...
<button onclick="fetchData({{ $alat->id_mesin }})" style="background-color: blue; color: white; padding: 4px 8px; border: none; border-radius: 5px; cursor: pointer;">Update</button>
            ...
            <div style="background: red; position: relative; width: 100px; height: 100px; display: flex; flex-direction: column; align-items: flex-end; justify-content: start;">
                ...
                <div style="margin-top: 50px; text-align: right;">
                    <p style="margin: 0;"><span id="lastUpdateSuhu_{{$alat->id_mesin}}"></span></p>
                    <p style="margin: 0;"><span id="lastUpdateKelembaban_{{$alat->id_mesin}}"></span></p>
                </div>
            </div>
...
function updateAlats() {
    ...
    function fetchLastUpdate{{$alat->id_mesin}}() {
    $.ajax({
        url: '/lastupdate', // Memanggil function yang sudah ada di Controller
        method: 'GET',
        dataType: 'json',
        success: function(data) {
            if (data) {
                // Update tampilan untuk semua alat
                @foreach($alats as $alat)
                    // document.getElementById("lastUpdate_{{ $alat->id_mesin }}").textContent = data.waktu;
                    document.getElementById("lastUpdateSuhu_{{ $alat->id_mesin }}").textContent = "Suhu: " + data.suhu + "°C";
                    document.getElementById("lastUpdateKelembaban_{{ $alat->id_mesin }}").textContent = "Kelembaban: " + data.kelembaban + "%";
                @endforeach
            } else {
                @foreach($alats as $alat)
                    document.getElementById("lastUpdate_{{ $alat->id_mesin }}").textContent = "Tidak ada data";
        ...
        error: function() {
            console.error("Gagal mengambil data terakhir.");
        }
    });
}

fetchLastUpdate{{$alat->id_mesin}}();
setInterval(fetchLastUpdate{{$alat->id_mesin}}, 2000);
