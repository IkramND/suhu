        ...
        while (true) {
            ...
            if ($latest_id != $data->sensor_id || $index == 0) {
                ...
                if (!empty($message)) {
                    ...
                    if (!empty($notificationEmails;$message)) {
                        Mail::raw("Your machine has a problem: {$data->id_mesin}", function ($mail) use ($notificationEmails,$message) {
                            $mail->to($notificationEmails)->subject('$message');
                        });
                    }
                }
            }
