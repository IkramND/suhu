        ...
        while (true) {
            ...
            if ($latest_id != $data->sensor_id || $index == 0) {
                ...
                if (!empty($message)) {
                    ...
                    $response = Http::timeout(5)->post("https://api.telegram.org/bot" . env('TELEGRAM_BOT_TOKEN') . "/sendMessage", [
                        'chat_id' => env('TELEGRAM_CHAT_ID'),
                        'text' => $message,
                        'parse_mode' => 'Markdown'
                    ]);

                    Log::info("🔍 TELEGRAM_BOT_TOKEN: " . env('TELEGRAM_BOT_TOKEN'));
                    Log::info("🔍 TELEGRAM_CHAT_ID: " . env('TELEGRAM_CHAT_ID'));
