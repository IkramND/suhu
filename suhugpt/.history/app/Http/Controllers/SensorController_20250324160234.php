    ...
    public function fetchData(Request $request)
{
    ...
    $second = $request->input('second','900'); // Default 900 detik (15 menit)

    $data = DB::table('data_sensor')
        ->where('id_mesin', $id_mesin)
        ->whereRaw('MOD(TIME_TO_SEC(TIME(waktu)) - TIME_TO_SEC(TIME((SELECT MIN(waktu) FROM data_sensor where id_mesin = ?))), ?) = 0', [$second,$id_mesin]) // Filter waktu berdasarkan interval detik
...
//     $data = DB::table('data_sensor')
//         ->where('id_mesin', $id_mesin)
//         ->whereRaw('MOD(id,?) = 0', [$second]) // Filter waktu berdasarkan interval detik
//         ->orderBy('waktu', 'desc') // Ambil data terbaru dulu
//         ->limit($limit) // Ambil data terbatas sesuai permintaan
//         ->get();
...
//     $data = DB::table('data_sensor')
//         ->where('id_mesin', $id_mesin)
//         ->whereRaw('MOD(TIME_TO_SEC(TIME(waktu)), ?) = 0', [$second]) // Filter waktu berdasarkan interval detik
//         ->orderBy('waktu', 'desc') // Ambil data terbaru dulu
//         ->limit($limit) // Ambil data terbatas sesuai permintaan
//         ->get();
...
//     $second = $request->input('second', '10');
