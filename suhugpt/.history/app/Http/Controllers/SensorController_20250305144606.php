...
{
    \Log::info('Request data:', $request->all()); // Log request ke storage/logs/laravel.log

    $id_mesin = $request->input('id_mesin');
    $limit = $request->input('limit', 10); // Default 10 jika tidak ada input
    $second = $request->input('second', 10); // Default 10 jika tidak ada input

    \Log::info("Limit: $limit, Second: $second"); // Log nilai yang terbaca

    $data = DB::table('data_sensor')
        ->where('id_mesin', $id_mesin)
        ->whereRaw('MOD(TIME_TO_SEC(TIME(waktu)), ?) = 0', [$second])
        ->orderBy('waktu', 'desc')
        ->limit($limit)
        ->get();

    return response()->json(collect($data)->reverse()->values());
}
