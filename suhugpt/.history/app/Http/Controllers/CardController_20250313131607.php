    ...
    $alats = DB::select("
        SELECT a.*,
               CASE
                   ...
                   ELSE 'Inactive'
               END AS status,
               COALESCE(b.latest_update, 'No Data') AS latest_update,
               COALESCE(b.suhu, 'No Data') AS suhu,
               COALESCE(b.kelembaban, 'No Data') AS kelembaban
        FROM alat a
        LEFT JOIN (
            SELECT ds1.id_mesin, ds1.waktu AS latest_update, ds1.suhu, ds1.kelembaban
            FROM data_sensor ds1
            WHERE ds1.waktu = (
                SELECT MAX(ds2.waktu)
                FROM data_sensor ds2
                WHERE ds2.id_mesin = ds1.id_mesin
                AND ds2.waktu BETWEEN NOW() - INTERVAL 5 MINUTE AND NOW()
            )
        ) b ON a.id_mesin = b.id_mesin;
    ");
