...
<head>
  ...
  <style>
    ...
    .dropdown-options {
        ...
        max-height: 200%;
        overflow-y: auto;
        z-index: 9999; /* Menambahkan z-index untuk memastikan dropdown di atas elemen lainnya */
        display: none;
        list-style: none;
        margin: 0;
        padding: 0;
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    dropdowns.forEach(function(dropdown) {
        ...
        if (!selected || !options || !hiddenInput || !arrow || !selectedText) {
            console.error("Dropdown elements missing or not found.");
            return;
        }
    ...
    // Ensure the year dropdown is populated after the page loads
    ...
    if (yearOptions) {
        ...
        // Select the first year by default, if year dropdown exists
        const firstYear = yearOptions.querySelector('li');
        if (firstYear) {
            const yearDropdown = document.querySelector('.custom-dropdown:last-child .dropdown-selected');
            if (yearDropdown) {
                yearDropdown.querySelector('.selected-text').textContent = firstYear.textContent; // Update the text of year dropdown
                yearDropdown.querySelector('input[type="hidden"]').value = firstYear.getAttribute('data-value'); // Set the hidden input value
            } else {
                console.error(".custom-dropdown:last-child .dropdown-selected not found!");
            }
        }
    } else {
        console.error('#year-options not found!');
    }
