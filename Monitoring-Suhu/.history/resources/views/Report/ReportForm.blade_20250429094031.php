...
<div class="login-dark">
  <form action="{{ route('report.result') }}" method="POST">
    ...
    <div class="form-group custom-dropdown">
        <div class="dropdown-selected"><span class="selected-text">Choose Year</span><span class="dropdown-arrow">&#9660;</span></div>
        <ul class="dropdown-options" id="year-options"></ul>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Script untuk memilih tahun
    const currentYear = new Date().getFullYear();
    const startYear = 2025;
    const endYear = currentYear + 5;
    const yearOptions = document.getElementById('year-options');
    const yearDropdown = document.querySelector('.custom-dropdown:last-child .dropdown-selected');

    // Log untuk memastikan apakah year-options sudah ada di DOM
    console.log(yearOptions);  // Debugging log

    // Memastikan tahun-tahun dimasukkan setelah dropdown tersedia di DOM
    setTimeout(function () {
        console.log('Menambahkan tahun...');  // Debugging log
        ...
        // Menambahkan event listener untuk memilih tahun
        yearOptions.querySelectorAll('li').forEach(function (option) {
            option.addEventListener('click', function (e) {
                e.stopPropagation();
                yearDropdown.querySelector('.selected-text').textContent = this.textContent; // Ganti teks dropdown tahun
                yearDropdown.querySelector('input[type="hidden"]').value = this.getAttribute('data-value'); // Simpan nilai di input hidden
                yearOptions.style.display = 'none';
                yearDropdown.querySelector('.dropdown-arrow').innerHTML = '&#9660;';
            });
        });

        console.log('Tahun ditambahkan ke dropdown');  // Debugging log
    }, 0);  // Settimeout memastikan dropdown dapat terisi setelah DOM dimuat
